...
void fill_amp_curve_specifier(double *array, t_atom *specifier, long num_specifiers)
{
  ...
  for(i=0;i<num_specifiers;i++) {
    ...
    //post("fill_amp_curve = %f", array[i]);
  }

  array[i] = HUGE_VAL;
  //post("fill_amp_curve_a = %f",array[i]);
}
		  ...
		  double *amp_curve)
{
  ...
  for(i=0; amp_curve && (i < 32); i++) {
    //post("amp[%d] = %f ", i, amp_curve[i]);
    if(isinf(amp_curve[i]) || amp_curve==NULL) /* */
      break;
  }
  ...
  /* x->amp_specifier[2] = num_items ? amp_curve[2] : 0; */
  //post("amp_spec0 = %f amp_spec1 = %f",x->amp_specifier[0],
  //     x->amp_specifier[1]);

  for(i=0; i<num_items; i++) {
    x->amp_specifier[2*i + 2] = log(amp_curve[2*i] / (f1 * sample_rate));
    x->amp_specifier[2*i + 3] = last_db_val = amp_curve[2*i + 1];
    /* post("last_db_val = %f", last_db_val); */
  }
...
t_uint ess_igen_dsp(t_ess *x, t_sample *out, t_uint startN, t_uint N, t_bool inv_amp)
{
  ...
  //post("amp_specifier = %p",amp_specifier);
  ...
  //post("j1 = %d", j);
  //post("num_amp = %d", x->num_amp_specifiers);
  //post("amp0 = %f amp1 = %f amp2 = %f amp3 = %f", amp_specifier[j], amp_specifier[j+1], amp_specifier[j+2], amp_specifier[j+3]);
...
static void irmeasure_process(t_irmeasure_tilde *x)
{
  ...
  if(bandlimit) {
    ...
    /* int pp=0; */
    /* for(pp=44100;pp<44108;pp++) */
    /*   post("sig = %f", excitation_sig[pp]); */
...
static void *irmeasure_tilde_new(t_symbol *s, short argc, t_atom *argv)
{
  ...
  x->test_tone = 0;
  x->no_dsp = 1;
  /* fissata la compensazione in ampiezza dei rumori */
  x->sample_rate = sys_getsr();
